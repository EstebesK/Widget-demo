...
import { statesData } from './../../Helpers/States';
...
const Payment = ({ next, back, total, formSummary, setPaymentForm }) => {
    ...
    useEffect(() => {
        if (statesId) {
            setCities(citiesData[statesId - 1])
        } else {
            setCities([])
        }
    }, [statesId])
    ...
    const { register, handleSubmit, formState: { errors }, ...methods } = useForm({
                    ...
                    <Grid container direction='column' spacing={2}>
                        ...
                        <Grid item>
                            <Grid container direction='row' justify='space-between' alignItems='center'>
                                <Grid item>
                                    <Typography variant='body1' style={riderDetails ? { color: '#FFFFFF' } : null}>Is passenger a cardholder?</Typography>
                                </Grid>
                                <Grid item>
                                    <Switch checked={riderDetails} onClick={() => setRiderDetails(!riderDetails)} color='primary' />
                                </Grid>
                            </Grid>
                        </Grid>
                        {!riderDetails &&
                            <Grid item style={{ paddingBottom: '20px' }}>
                                ...
                                <Grid item>
                                    <Grid container direction='row' justify='space-between' spacing={2}>
                                        ...
                                        <Grid item xs={6}>
                                            <CustomFormInput variant='outlined'
                        ...
                        <Grid item>
                            ...
                            <Grid container direction='row' justify='space-between' spacing={2}>
                                <Grid item xs={6}>
                                    ...
                                    {errors.client?.firstName && <p className={classes.error}>{errors.client?.firstName.message}</p>}
                                </Grid>
                                <Grid item xs={6}>
                                    <CustomFormInput
                                        variant='outlined'
                                    ...
                                    {errors.client?.lastName && <p className={classes.error}>{errors.client?.lastName.message}</p>}
                                </Grid>
                            </Grid>
                        </Grid>
                        <Grid item>
                            <Grid container direction='row' justify='space-between' spacing={2}>
                                <Grid item xs={6}>
                                    ...
                                    />
                                    {errors.client?.email && <p className={classes.error}>{errors.client?.email.message}</p>}
                                </Grid>
                                <Grid item xs={6}>
                                    <CustomFormInput variant='outlined'
                                    ...
                                    {errors.client?.phoneNumber && <p className={classes.error}>{errors.client?.phoneNumber.message}</p>}
                                </Grid>
                            </Grid>
                        </Grid>
                        <Grid item>
                            ...
                            {errors.client?.address && <p className={classes.error}>{errors.client?.address.message}</p>}
                        </Grid>
                        <Grid item >
                            <Autocomplete
                                id="combo-box-demo"
                                ...
                                renderInput={params => (
                                    <TextField
                                        {...params}
                                        fullWidth placeholder="State" variant="outlined"
                                    />
                                )}
                            ...
                            <Grid container direction='row' justify='space-between' spacing={2}>
                                <Grid item xs={6}>
                                    <Autocomplete
                                        ...
                                        renderInput={params => (
                                            <TextField
                                                {...params}
                                                fullWidth placeholder="Cities" variant="outlined"
                                            />
                                        )}
                                        onChange={(event, newValue) => {
                                ...
                                <Grid item xs={6}>
                                    <CustomFormInput variant='outlined'
                                        ...
                                        error={errors.client?.address ? true : false}
                                    />
                                    {errors.client?.zip && <p className={classes.error}>{errors.client?.zip.message}</p>}
                                </Grid>
                            </Grid>
                        </Grid>
                        <Grid item>
                        ...
                        <Grid item>
                            <CustomMaskInput
                                name='paymentInfo.cardNumber' mask="9999-9999-9999-9999"
                            ...
                            {errors.paymentInfo?.cardNumber && <p className={classes.error}>{errors.paymentInfo?.cardNumber.message}</p>}
                        </Grid>
                        <Grid item>
                            <Grid container direction='row' justify='space-between' spacing={2}>
                                <Grid item xs={6}>
                                    <CustomMaskInput
                                        name='paymentInfo.month' mask='99/99'
                                    ...
                                    {errors.paymentInfo?.month && <p className={classes.error}>{errors.paymentInfo?.month.message}</p>}
                                </Grid>
                                <Grid item xs={6}>
                                    <CustomMaskInput
                                        name='paymentInfo.cvc' type='date' mask="999"
                                        defaultValue={formSummary.paymentInfo.cvc}
                                    >
                                        {() => <TextField variant='outlined' placeholder='CVV/CVC' fullWidth
                                            error={errors.paymentInfo?.cvc ? true : false} />}
                                    </CustomMaskInput>
                                    {errors.paymentInfo?.cvc && <p className={classes.error}>{errors.paymentInfo?.cvc.message}</p>}
                                </Grid>
                            </Grid>
                        </Grid>
                        <Grid item>
                            <Checkbox
                                color="default" onClick={() => setChecked(!checked)}
                            />
                            <Link underline='always' style={{ color: '#BABABA' }}
                            ><TermsOfUse />
                            </Link> / <Link underline='always' style={{ color: '#BABABA' }}>
                                <PrivacyPolicy />
                            </Link>
                        </Grid>
                        <Grid item>
                            <Grid container direction='row' alignItems="center" justify='center' spacing={1} className={classes.buttonGroup}>
                                ...
                                <Grid item xs={6}>
                                    <Button variant='contained' fullWidth
                                        type='submit'
                                        color='primary'
                                        style={{ height: '50px', borderRadius: '8px', textTransform: 'none' }}
                                        disabled={!checked}>Pay ${total}</Button>
                                </Grid>
                            </Grid>
                        </Grid>
                    </Grid>
